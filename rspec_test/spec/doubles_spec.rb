...
describe 'Doubles' do
	...
	it 'allow stubbing multiple response with #and_return' do
		die = double('Die')
	...
	context 'with partial test doubles' do
		...
		it 'allows stubbing instance methods on Ruby classes' do
			time = Time.new(2017, 1, 1, 12, 0, 0)
			allow(time).to receive(:year).and_return(1975)

			expect(time.to_s).to eq('2017-01-01 12:00:00 +0300')
		...
		it 'allows stubbing class methods on Ruby classes' do
			fixed = Time.new(2017, 1, 1, 12, 0, 0)
			allow(Time).to receive(:now).and_return(fixed)

			expect(Time.now).to eq(fixed)
			expect(Time.now.to_s).to eq('2017-01-01 12:00:00 +0300')
	...
	context 'with message expectation' do
	...
	context 'with message count constraints' do
		...
		it 'allows constraints on message' do
			class Cart
